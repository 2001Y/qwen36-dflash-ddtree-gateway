...
Youssofal でも 35B-A3B / MLX / 4bit / Heretic 系なので、同じ DFlash draft を明示して DFlash / DDTree 経路で起動できます。ただし手元ベンチでは `DFlash + Youssofal` は `4/25`、`DDTree + Youssofal` は `11/25` まで有効で `6/25` 成功の部分評価です。総合採用判断ではまだ TheCluster を上位にしています。
...
- 参考結果: `DDTree + Youssofal` は `11/25` まで有効、`6/25` 成功。`house` 以降を rest2 で再評価中
