...
`DFLASH_GATEWAY_IDLE_SECONDS=1800` が標準です。最後に `/v1/*` の生成系リクエストを転送してから 30 分間アクセスがなければ、gateway が backend process group に `SIGTERM` を送り、必要なら `SIGKILL` します。

TheCluster の手元実測では、local cache 済みの backend 起動は約 `9.1s`、最小 API リクエスト全体は約 `13.5s` でした。保持中は `mlx_active` が約 `20GB` になります。メモリ圧迫が問題ない運用では、ロードを繰り返すより 30 分ほど保持する方が体感と電力面のバランスが良いです。RAM をすぐ戻したい場合だけ、`.env` で `DFLASH_GATEWAY_IDLE_SECONDS=30` などに下げてください。
