...
| ---: | --- | ---: | ---: | --- | --- | --- |
| 1 | `Youssofal/Qwen3.6-35B-A3B-Abliterated-Heretic-MLX-4bit` | `21,866` | `16` | `false` | `false` | `DFlash` は `12/25` まで有効、`4/25` 成功。`DDTree` は `11/25` まで有効、`6/25` 成功。`food-chain` 後に orphan cleanup。`house` 以降を rest2 で継続 |
...
- `DFlash + Youssofal` は `4/25`、`DDTree + Youssofal` は `6/25` まで確定。DDTree は `food-chain` を通常失敗として復元済みで、`house` 以降を 1 exercise = 1 server で再評価中です。
