...
STAMP="$(date '+%Y%m%d-%H%M%S')-youssofal-ddtree-rest2-invalid-rerun-per-exercise"
...
{
  echo "[$(date '+%Y-%m-%dT%H:%M:%S%z')] starting Youssofal DDTree rest2 invalid rerun per-exercise"
...
if [[ ! -x "$BENCH_PYTHON" ]]; then
  echo "[$(date '+%Y-%m-%dT%H:%M:%S%z')] missing BENCH_PYTHON=$BENCH_PYTHON" >> "$RUNNER_LOG"
  exit 127
fi

if [[ ! -d /private/tmp/ts-bench/exercism-typescript/exercises/practice ]]; then
  echo "[$(date '+%Y-%m-%dT%H:%M:%S%z')] missing ts-bench exercism-typescript submodule" >> "$RUNNER_LOG"
  exit 126
fi
