...
def generate_ddtree_once(
    *,
    ...
    prefill_logits, prefill_hidden = target_adapter.forward_with_hidden_capture(
        input_ids=prompt_array,
        cache=target_cache,
        capture_layer_ids=capture_layer_ids,
    )
    eval_logits_and_captured(prefill_logits, prefill_hidden)
    ...
    target_hidden = target_adapter.extract_context_feature(
    ...
    verify_len_cap = target_adapter.resolve_verify_len_cap(block_size)
    ...
    def _run_dflash_cycle(block_len: int) -> tuple[int, bool, float]:
        ...
        if block_len > 1:
            ...
            noise_embedding = target_adapter.embed_tokens()(block_token_ids[None])
            draft_hidden = draft_model(
                noise_embedding=noise_embedding,
                target_hidden=target_hidden,
                cache=draft_cache,
            )
            draft_logits = target_adapter.logits_from_hidden(draft_hidden[:, 1:, :])
        ...
        target_adapter.arm_rollback(target_cache, prefix_len=start)

        verify_start_ns = time.perf_counter_ns()
        verify_logits, verify_hidden_raw = target_adapter.verify_block(
        ...
        acceptance_len = int(
            match_acceptance_length(verify_token_ids[1:], posterior[:-1]).item()
        )
        commit_count = 1 + acceptance_len
        committed_segment = verify_token_ids[:commit_count]
        committed_hidden = target_adapter.extract_context_feature(
            verify_hidden_raw,
        ...
        replay_ns = target_adapter.restore_after_acceptance(
            target_cache,
            target_len=start,
    ...
    while len(generated_tokens) < max_new_tokens:
        ...
        if draft_logits is None or block_len <= 1:
            ...
            fwd_logits, fwd_hidden = target_adapter.forward_with_hidden_capture(
                input_ids=staged_first[None] if staged_first.ndim == 1 else staged_first[None, None],
                cache=target_cache,
                capture_layer_ids=capture_layer_ids,
            )
            eval_logits_and_captured(fwd_logits, fwd_hidden)
            target_hidden = target_adapter.extract_context_feature(
        ...
        # --- COMMIT ---
        commit_start = time.perf_counter_ns()
        all_hidden = target_adapter.extract_context_feature(
        ...
        if tree_aware_linear and exact_commit:
            ...
            commit_logits, commit_hidden_raw = target_adapter.forward_with_hidden_capture(
                input_ids=commit_ids_mx,
                cache=target_cache,
                capture_layer_ids=capture_layer_ids,
            )
            eval_logits_and_captured(commit_logits, commit_hidden_raw)
            committed_hidden = target_adapter.extract_context_feature(
            ...
            suffix_logits, suffix_hidden_raw = target_adapter.forward_with_hidden_capture(
                input_ids=suffix_ids_mx,
                cache=target_cache,
                capture_layer_ids=capture_layer_ids,
            )
            eval_logits_and_captured(suffix_logits, suffix_hidden_raw)
            hidden_chunks.append(
                target_adapter.extract_context_feature(
