...
from ddtree_mlx.dflash_compat import (
    get_stop_token_ids,
...
async def chat_completions(request: Request):
    ...
    chat_template_kwargs = payload.get(
        ...
        payload.get("chat_template_args", {"enable_thinking": False}),
    )
    ...
    default_max_tokens = int(os.environ.get("DDTREE_DEFAULT_MAX_TOKENS", "4096"))
    max_tokens = int(
        payload.get("max_tokens", payload.get("max_completion_tokens", default_max_tokens))
    )
...
if __name__ == "__main__":
    ...
    print(
        f"DDTree server ready: {args.model} + {draft_ref or resolve_draft_ref(args.model) or 'no draft'}"
    )
